...
if [ "$(readlink $dev_dir/pylint)" != "$git_root/pylint" ]; then
    rm -rf "$dev_dir/pylint/plugins"
    cp -r "$git_root/pylint/plugins" "$dev_dir/pylint/plugins"
fi

# Symlink integration's custom_components folder into checked out code to enable hassfest.
if [ "$(readlink $git_root/homeassistant/components/$integration_name)" != "$dev_dir/custom_components/$integration_name" ]; then
    ln -s $dev_dir/custom_components/$integration_name $git_root/homeassistant/components/$integration_name
fi
